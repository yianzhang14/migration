...
- The `grav_model.ipynb` is used to train the reference gravity model used to benchmark the logit model. It is currently a bit buggy.
- The `simulation.ipynb` notebook is used to run inference. It takes in data files and estimated coefficients and computes utilities, translating them into probabilities. It is currently unreviewed but should work properly in theory.
